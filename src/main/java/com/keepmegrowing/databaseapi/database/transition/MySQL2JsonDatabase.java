...
public class MySQL2JsonDatabase
implements DatabaseSetup {
    @Override
    public <T> AbstractDatabaseHandler<T> getHandler(Class<T> clazz, DatabaseSettings databaseSettings) {
        return new TransitionDatabaseHandler<T>(clazz, new MySQLDatabase().getHandler(clazz, databaseSettings), new JSONDatabase().getHandler(clazz, databaseSettings));
    }
}
