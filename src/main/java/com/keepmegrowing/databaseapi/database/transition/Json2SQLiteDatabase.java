...
public class Json2SQLiteDatabase
implements DatabaseSetup {
    @Override
    public <T> AbstractDatabaseHandler<T> getHandler(Class<T> clazz, DatabaseSettings databaseSettings) {
        return new TransitionDatabaseHandler<T>(clazz, new JSONDatabase().getHandler(clazz, databaseSettings), new SQLiteDatabase().getHandler(clazz, databaseSettings));
    }
}
